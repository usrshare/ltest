...
//this file is based on creature/creaturenames.cpp from Liberal Crime Squad

//names have been modified to replace accented characters with ASCII ones.
...
static const char *male_first_names[] =
{
	...
	"Gabor",   "Gabriel",   "Garett",   "Garreth",
	"Garth",   "Gary",   "George",   "Gerald",
	"Geraldo",   "Geza",   "Gilbert",   "Giovanni",
	"Glen",   "Glenn",   "Graham",   "Greg",
	"Gregory",   "Gus",   "Gustav",   "Guy",
	"Gyorgy",   "Gyula",   "Hafez",   "Hagar",
	...
	"Samuel",   "Sander",   "Sandor",   "Sanford",
	"Scott",   "Sean",   "Sebastian",   "Serge",
...
static const char *regular_last_names[] =
{
	...
	"Hornberger", "Horvath", "Houghton", "Houseman",
	"Howard", "Howe", "Hsu", "Hu",
	...
	"Juhasz", "Justice", "Kagan", "Kaltenbrunner",
	...
	"Kobayashi", "Kokernot", /*"Koprowski",*/ "Kovacs",
	...
	"Rabban", "Rackard", "Racz", "Radman",
	"Rainey", "Ramirez", "Ramos", "Randell",
	...
	"Saito", "Sanchez", "Sanders", "Sansome",
	"Santiago", "Santos", "Sasaki", "Sato",
	"Savedra", "Saxton", "Scales", "Scardino",
	"Schmitt", "Schneider", "Schroeder", "Schmidt",
...
//these are the foreign last names that change dependent on whether the person
//is male or female (like in Slavic countries). The first column lists the male version,
//the second lists the female version
//this list is based on one Russian guy's personal experience/judgement and most likely needs fixing
static const char *gender_dependent_last_names[][2] = {
...
static const char *archconservative_last_names[] =
{
	...
	"Gibson", "Gingrich", "Giuliani", "Gluecksburg",
...
static const char *gender_dependent_archcon_last_names[][2] = {
	{"Kuklinski","Kuklinska"},
	{"Romanov","Romanova"}
};

#define arraylen(x) (sizeof(x)/sizeof(*x)) //for random name lists

int random_first_name(char* dest, enum entity_gender gender) {

	//by this point, gender should NOT be EG_RANDOM, EG_MBIAS or EG_FBIAS.
	//if it happens to be this way, though, this function will return
	//a gender-neutral name.

	enum entity_gender name_gender = EG_NEUTRAL;
	...
	switch(name_gender) {
		case EG_MALE:
			randname = male_first_names[randval(arraylen(male_first_names))];
			break;
		case EG_FEMALE:
			randname = female_first_names[randval(arraylen(female_first_names))];
			break;
		case EG_WMPATRIARCH:
			randname = great_white_male_patriarch_first_names[randval(arraylen(great_white_male_patriarch_first_names))];
			break;
		case EG_NEUTRAL:
		default:
			randname = gender_neutral_first_names[randval(arraylen(gender_neutral_first_names))];
			break;
	}

	strncpy(dest,randname,16);
	return 0;
}

int random_last_name(char* dest, bool archcon,enum entity_gender gender) {

	//non-archcon characters get their names from both lists.
	//archcon characters only use the archcon list.

	int gender_col = randval(2);
	bool use_gender = 0;
	...
	bool use_archcon = archcon;
	if ((unsigned int)randval(arraylen(regular_last_names) + arraylen(archconservative_last_names)) > arraylen(regular_last_names)) use_archcon = 1;
